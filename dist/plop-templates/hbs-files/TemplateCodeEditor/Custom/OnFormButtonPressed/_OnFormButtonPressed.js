 ...
 * @param {*} setCurrentState, setter de state dUI
 * @param {*} currentState, le state de lUI
 ...
 */
export async function _OnFormButtonPressed({ prompt, currentState, answers, route, onInput }) {
    /* PLOP_INJECT_CODE */


    // indicates that the async work has started (or rather is about to be started)
    OnStartLoading({ currentState, onInput, prompt });
    ...
    const asyncCake = await BakeAsyncCake({ prompt, currentState, answers, route, onInput })
