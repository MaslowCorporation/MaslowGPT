 ...
 */
export async function OnFormButtonPressed({ prompt, currentState, answers, route, onInput }) {
    // error safety net baby !
    try {
        await _OnFormButtonPressed({ prompt, currentState, answers, route, onInput })
    } catch (e) {
        // indicate that an error took place during the process
        // save that in the state
        OnDataLoadingFailed({ currentState, onInput });
    }
}
